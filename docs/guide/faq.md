  ...
  <section class="brand-card">
    ...
    <div class="brand-grid brand-grid--two">
      ...
      <article class="brand-card brand-card--nested">
        ...
        <p>不是只能统一一套。平台可以维护默认配置，正式租户也可以维护自己的本地配置；如果租户本地配置不完整，运行时会继续回退平台默认配置。现在公开口径还要再补一层：平台超管和租户管理员都可以维护自己的大模型列表，并把模型授权给租户或具体用户；用户在公告中心确认后，模型才会进入自己的对话执行可选范围。为了避免敏感信息横向暴露，完整 Token 只对配置方可见，另一侧只显示遮罩。</p>
      </article>
      <article class="brand-card brand-card--nested">
